...
<head>
    ...
    <title>Message To Student</title>
            ...
            <div class="col-lg-10 border rounded offset-lg-1 p-3 mt-5">
                <div class="row">
                    ...
                    if (!empty($_GET["i1"])&&!empty($_GET["i2"])) {
                        ...
                        $t = Database::search("select * from teacher where id='" . $teacher_id . "'");
                        $n = $r->num_rows;
                        $rs = $r->fetch_assoc();
                        $ts = $t->fetch_assoc();
                        $r1 = Database::search("select * from messages where `to`='".$rs["email"]."' and `from`='".$ts["email"]."' order by date_time DESC");
                        $n1 = $r1->num_rows;
                        if ($n1 > 0) {
                            for ($x = 0; $x < $n1; $x++) {
                                $rs1 = $r1->fetch_assoc();
                ...
                <div class="col-12 mt-5">
                    <div class="row">
                        <div class="input-group">
                            ...
                            <button id="sendbtn" class="btn btn-link fs-2 bg-dark" onclick="teacherMessageToStudent('<?=$rs['email']?>','<?=$ts['email']?>');">
